...
static inline MirProjection *MirProjection_new_field(struct Mir *mir, int index)
{
    MirProjection *proj = MirProjection_new(mir);
...
inline static struct MirInstruction *pawMir_new_unary_op(struct Mir *mir, struct SourceLoc loc, enum UnaryOp op, struct MirPlace val, struct MirPlace output)
{
    struct MirInstruction *instr = pawMir_new_instruction(mir);
...
inline static struct MirInstruction *pawMir_new_binary_op(struct Mir *mir, struct SourceLoc loc, enum BinaryOp op, struct MirPlace lhs, struct MirPlace rhs, struct MirPlace output)
{
    struct MirInstruction *instr = pawMir_new_instruction(mir);
