...
//
// The register allocator uses the standard "linear scan" algorithm, adapted
// to produce a register mapping that satisfies certain constraints imposed
// by Paw's runtime. Register constraints include the following:
// (1) If an instruction might cause a memory allocation, then the register
//     containing the allocated object must be the topmost physical register.
//     The GC might collect any objects above this register.
// (2) Call instructions expect the function object on top of the stack,
//     followed by the arguments. The function object is replaced with the
//     return value before control is returned back to the caller.
// (3) Range getters and setters expect the "upper" argument to be in the
//     physical register directly above the "lower" argument.
//
//     TODO: there are other concerns for setters, since they might
//           cause an allocation (even element setter for map container)
//           need to make sure the "rhs" is not collected. may be better
//           to use a special function instead.
//
...
struct RegisterAllocator {
    ...
    // resolve_registers:
    //   resolved: stores parallel copies generated during resolution and indicates
    //             which blocks have already been visited
    Map *resolved; // MirBlock => struct ParallelCopy * | NULL
    ...
    //   inactive: intervals with a hold at "position"
...
//        K_LIST_SET(R.result, c.r.value, REGINFO(1 + nparameters + i));
//    }
