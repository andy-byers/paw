...
static struct RegisterInfo *new_reginfo(struct Compiler *C, struct MirRegister *r, int index)
{
    if (index > LOCAL_MAX) {
        pawE_error(ENV(C), PAW_EOVERFLOW, -1, "too many local variables (max is %d)", LOCAL_MAX);
    }
    struct RegisterInfo *info = pawK_pool_alloc(ENV(C), C->pool, sizeof(struct RegisterInfo));
...
static struct RegisterInfo *regalloc(struct RegisterAllocator *R, struct MirRegister *r)
{
    ...
    struct RegisterInfo *result = new_reginfo(R->C, r, R->free_reg++);
...
// TODO: obv. not ideal, O(N) in number of registers. track separately
//       the point is to free all intervening temporaries
    paw_Int iter = PAW_ITER_INIT;
...
Map *pawP_allocate_registers(struct Compiler *C, struct Mir *mir, struct MirBlockList *order)
{
    struct MirVisitor V;
