...
static Arena *new_arena(struct Pool *pool, size_t required_size)
{
    ...
    if (size0 > SIZE_MAX / 2)
        pawM_error(pool->P); // sanity check

    size_t const size = PAW_MAX(size0 * 2, required_size);
    Arena *a = pawM_new_flex(pool->P, Arena, size, 1);
    a->size = size;
    return a;
}
...
{
    pool->P = P;
    pool->arena = new_arena(pool, base_size);
    pool->full = NULL;
    pool->st = st;
}
...
{
    free_arena_list(pool->P, pool->arena);
    free_arena_list(pool->P, pool->full);
}
...
static void *pool_malloc(struct Pool *pool, size_t size)
{
    paw_assert(size > 0);
    ...
    Arena *const a = new_arena(pool, size);
    a->prev = pool->arena;
    pool->arena = a;
    a->used = size;
    return a->data;
...
void *pawK_pool_alloc(struct Pool *pool, void *ptr, size_t size0, size_t size)
{
    ...
    void *ptr2 = pool_malloc(pool, size);
    if (size0 > 0) {
