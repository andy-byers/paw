...
static struct IrType *collect_type(struct MonoCollector *M, struct IrType *type);
...
struct IrType *finalize_type(struct MonoCollector *M, struct IrType *type)
{
    ...
    type = collect_type(M, type); // TODO pawIr_fold_type(&M->F->F, type);
    if (!IrIsSignature(old_type)) return type;
    struct IrSignature *t = IrGetSignature(old_type);
    if (!pawP_is_assoc_fn(M->C, t)) return type;
...
static void CopySwitch(struct MonoCollector *M, struct MirSwitch *t, struct MirSwitch *r)
{
    ...
    for (int i = 0; i < t->arms->count; ++i) {
        ...
        K_LIST_PUSH(M->C, r->arms,
                pawMir_new_switch_arm(M->C, arm->value, arm->bid));
    }
}
...
static struct MirUpvalueInfo *new_upvalue_info(struct MonoCollector *M)
{
    return pawK_pool_alloc(ENV(M->C), M->C->pool, sizeof(struct MirUpvalueInfo));
}
...
static void do_monomorphize(struct MonoCollector *M, struct Mir *base, struct Mir *inst, struct IrType *self)
{
    ...
    for (int i = 0; i < base->upvalues->count; ++i) {
        ...
        struct MirUpvalueInfo *dst = new_upvalue_info(M);
        *dst = *src;
        K_LIST_PUSH(M->C, inst->upvalues, dst);
    }
...
{
    struct GenericsState gs;
    enter_generics_context(M, &gs, ir_adt_types(base->self), ir_adt_types(self));
...
static struct IrType *register_method(struct MonoCollector *M, struct IrSignature *t, struct IrType *self)
{
...
static struct IrType *collect_body_type(struct MonoCollector *M, struct IrSignature *t)
{
...
{
    for (int i = 0; i < monos->count; ++i) {
        struct IrType *type = K_LIST_GET(monos, i);
        if (test_types(M, type, type)) return type;
    }
    K_LIST_APPEND(M->C, M->types, type);
    K_LIST_APPEND(M->C, monos, type);
    return type;
}

static struct IrType *collect_function(struct IrTypeFolder *F, struct IrSignature *t)
{
    struct MirTypeFolder *outer = F->ud;
    struct MonoCollector *M = outer->ud;
    struct HirDecl *decl = pawHir_get_decl(M->C, t->did);

    if (!HirIsVariantDecl(decl)) return collect_body_type(M, t);
    return IR_CAST_TYPE(t);
}

static struct IrType *collect_adt(struct IrTypeFolder *F, struct IrAdt *t)
{
    struct MirTypeFolder *outer = F->ud;
    struct MonoCollector *M = outer->ud;
...
static struct IrType *collect_type(struct MonoCollector *M, struct IrType *type)
{
    ...
    if (IrIsAdt(type)) return collect_adt(&M->F->F, IrGetAdt(type));
    if (IrIsSignature(type)) return collect_body_type(M, IrGetSignature(type));
    return type;
}
...
struct MonoResult pawP_monomorphize(struct Compiler *C, Map *bodies)
{
    ...
    F.F.FoldSignature = collect_function;
    F.F.FoldAdt = collect_adt;
    ...
    while (pawH_iter(bodies, &iter)) {
        ...
        if (is_entrypoint(mir)) {
            mir->type = collect_body_type(&M, IrGetSignature(mir->type));
        }
    }
