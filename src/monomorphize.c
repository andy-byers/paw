...
static void CopyPhi(struct MonoCollector *M, struct MirPhi *x, struct MirPhi *r)
{
    ...
    for (int i = 0; i < x->inputs->count; ++i) {
        K_LIST_PUSH(M->C, r->inputs, K_LIST_GET(x->inputs, i));
    }
}
...
static struct MirTerminator *copy_terminator(struct MonoCollector *M, struct MirTerminator *terminator)
{
    struct MirTerminator *result = pawMir_new_terminator(M->C, terminator->hdr.kind);
    switch (MIR_KINDOF(terminator)) {
#define DEFINE_COPY(X) case kMir##X: \
            Copy##X(M, MirGet##X(terminator), MirGet##X(result)); \
            break;
...
static struct MirBlockData *copy_basic_block(struct MonoCollector *M, struct MirBlockData *block)
{
    ...
    for (int i = 0; i < block->instructions->count; ++i) {
        struct MirInstruction *r = copy_instruction(M, K_LIST_GET(block->instructions, i));
        K_LIST_PUSH(M->C, result->instructions, r);
    }
    result->terminator = copy_terminator(M, block->terminator);
    return result;
}
