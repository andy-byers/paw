...
//};
//
//DEFINE_MAP(struct Unboxer, LocalGroupMap, pawP_alloc, P_ID_HASH, P_ID_EQUALS, MirRegister, struct MirPlace)
//DEFINE_MAP(struct Unboxer, UpvalueGroupMap, pawP_alloc, P_PTR_HASH, P_PTR_EQUALS, int, struct MirPlace)
...
//}
//
////static struct MirPlace place_at(struct Unboxer *U, struct MirPlace group, int index)
////{
...
////}
//
//static struct MirPlace get_registers(struct Unboxer *U, MirRegister r, int discr);
//
...
//}
//
//static struct MirPlace new_registers(struct Unboxer *U, struct MirPlace place, IrType *type, int discr)
//{
//    struct FunctionState *fs = U->fs;
//    struct RegisterState rs = {0};
//    if (place.kind == MIR_PLACE_LOCAL) {
//        struct MirRegisterData const rdata = MirRegisterDataList_get(fs->registers, place.r.value);
...
//{
//    struct FunctionState *fs = U->fs;
//    struct MirPlace const *pgroup = LocalGroupMap_get(U, fs->local_map, r);
...
//    struct MirPlace const group = {
//        .type = rdata->type,
//        .kind = MIR_PLACE_LOCAL,
//        .r.value = num_registers,
//    };
//    LocalGroupMap_insert(U, fs->local_map, r, group);
//    return group;
//}
//
//static struct MirPlace get_upvalues(struct Unboxer *U, int up)
//{
//    struct MirPlace const *pgroup = UpvalueGroupMap_get(U, U->fs->upvalue_map, up);
//    paw_assert(pgroup != NULL);
//    return *pgroup;
//}
//
//static struct MirPlace new_nonlocal_upvalues(struct Unboxer *U, int up, struct MirPlace mg)
//{
...
//
//    struct UpvalueState us = {.up = mg.up};
...
//}
//
//static struct MirPlace new_local_upvalues(struct Unboxer *U, int up, struct MirPlace mg, int local)
//{
//    struct FunctionState *fs = U->fs;
...
//}
//
//static struct MirPlace get_location(struct Unboxer *U, struct MirPlace p)
//{
...
//}
//
////static struct MirPlace split_group(struct MirPlace group, int offset, int count)
////{
...
//    struct MirPlace const output = new_registers(U, pa->base, pa->field.type, pa->field.discr);
//
////    struct MirPlace const object = pa->base;
////    for (int i = 0; i < output.count; ++i) {
////        struct MirPlace const result = place_at(U, output, i);
////        NEW_INSTR(U, get_field, TODO, pa->field.offset + i, result, object);
////    }
//    NEW_INSTR(U, get_field, TODO, pa->field.offset, output, pa->base);
//
//    pa->base = output;
...
//    struct MirPlace const output = new_registers(U, pa->base, access_type, field_discr);
//
////    struct MirPlace const object = pa->base;
////    if (output.count > 1 || field_offset > 0) {
//        struct MirPlace const pointer = new_rawptr_place(U, element_type);
//        NEW_INSTR(U, get_element_ptr, TODO, kind, pointer, pa->base,
...
////        }
////
//        NEW_INSTR(U, unpack, TODO, field_offset, output, pointer);
//
////    } else {
...
//    struct FunctionState *fs = U->fs;
//    enum BuiltinKind const kind = pawP_type2code(U->C, pa->type);
//    struct MirPlace const output = new_registers(U, pa->base, pa->type, ENUM_BASE);
//
//    NEW_INSTR(U, get_range, TODO, kind, output, pa->base, pa->range.lower, pa->range.upper);
//
//    pa->base = output;
...
//{
//    struct MirPlace const output = new_registers(U, pa->base, pa->type, ENUM_BASE);
//    NEW_INSTR(U, upvalue, TODO, output, pa->base.up);
//    pa->base = output;
//}
//
...
//{
//    if (pa->base.kind == MIR_PLACE_UPVALUE)
...
//        struct MirPlace const index_group = get_registers(U, index->index, ENUM_BASE);
////        paw_assert(index_group.count >= 1);
//        pa->element.key = index_group;
...
//        pa->range.lower = get_registers(U, range->lower, ENUM_BASE);
//        pa->range.upper = get_registers(U, range->upper, ENUM_BASE);
...
//{
//    struct MirPlace const output = lhs.base;
//    struct MirPlace const value = rhs.base;
//    NEW_INSTR(U, move, TODO, output, value);
//}
//
//static void create_indirect_field_setter(struct Unboxer *U, struct MemoryAccess lhs, struct MemoryAccess rhs)
//{
////    struct MirPlace const object = lhs.base;
////    struct MirPlace const value = rhs.base;
...
////    }
//
//    NEW_INSTR(U, set_field, TODO, lhs.field.offset, lhs.base, rhs.base);
//}
//
...
//
//    struct Mir *mir = U->fs->mir;
////    struct MirPlace const object = place_at(U, lhs.base, 0);
...
////        for (int i = 0; i < rhs.base.count; ++i) MirPlaceList_push(mir, value, place_at(U, rhs.base, i));
//
//        NEW_INSTR(U, set_element, TODO, kind, lhs.base, lhs.element.key, rhs.base, field_offset);
////    } else {
////        struct MirPlace const value = place_at(U, rhs.base, 0);
...
//{
//    enum BuiltinKind const kind = pawP_type2code(U->C, lhs.type);
//    NEW_INSTR(U, set_range, TODO, kind, lhs.base, lhs.range.lower, lhs.range.upper, rhs.base);
//}
//
...
////        NEW_INSTR(U, set_upvalue, TODO, lhs.base.base + i, v);
////    }
//
//    NEW_INSTR(U, set_upvalue, TODO, lhs.base.up, rhs.base);
//}
//
...
//    } else if (lhs.base.kind == MIR_PLACE_UPVALUE) {
...
//{
//    struct MirPlace const output = get_location(U, x->output);
//    NEW_INSTR(U, alloc_local, x->loc, x->name, output);
//}
//
...
//
//        MirPlaceList_push(U->fs->mir, result, a.base);
//    }
//    return result;
//}
//
...
////    struct MirPlace const output = get_location(U, x->output);
////    NEW_INSTR(U, aggregate, x->loc, fields, output, PAW_TRUE);
////}
////
...
////
////        MirPlaceList_push(mir, inputs, field.base);
////    }
////
////    NEW_INSTR(U, aggregate, x->loc, inputs, output, PAW_FALSE);
////}
//
//static void unbox_aggregate(struct Unboxer *U, struct MirAggregate *x)
//{
////    struct Mir *mir = U->fs->mir;
////    struct MirRegisterData const *rdata = mir_reg_data(mir, x->output.r);
...
//    MirPlaceList *fields = discharge_ordered_fields(U, x->fields);
//    struct MirPlace const output = get_location(U, x->output);
//    NEW_INSTR(U, aggregate, x->loc, fields, output, x->is_boxed);
//}
//
...
//
////        for (int i = 0; i < elem.base.count; ++i) {
////            struct MirPlace const r = place_at(U, elem.base, i);
////            MirPlaceList_push(mir, elems, r);
////        }
//        MirPlaceList_push(mir, elems, elem.base);
//    }
//
//    struct MirPlace const object = get_location(U, x->output);
////    struct MirPlace const output = place_at(U, object, 0);
//    NEW_INSTR(U, container, x->loc, x->b_kind, x->elem_count, elems, object);
//}
//
...
//
////    paw_assert(callable.base.count == 1);
////    struct MirPlace const target = place_at(U, callable.base, 0);
//
...
////                struct MirPlace const r = place_at(U, a.base, i);
////                MirPlaceList_push(mir, args, r);
////            }
//
//            MirPlaceList_push(mir, args, a.base);
//        }
//    }
//
...
////        struct MirPlace const result = get_location(U, K_LIST_FIRST(x->outputs));
...
////    }
//
//    struct MirPlace const result = get_location(U, x->output);
//    NEW_INSTR(U, call, x->loc, callable.base, args, result);
//}
//
//
//static void unbox_capture(struct Unboxer *U, struct MirCapture *x)
//{
//    struct MirPlace const target = get_location(U, x->target);
////    for (int i = 0; i < target.count; ++i) {
////        struct MirPlace const r = place_at(U, target, i);
////        NEW_INSTR(U, capture, x->loc, r);
////    }
//    NEW_INSTR(U, capture, x->loc, target);
//}
//
...
////    }
//    NEW_INSTR(U, close, x->loc, target);
//}
//
...
////    paw_assert(value.base.count == 1);
//    x->val = value.base;
////    place_at(U, value.base, 0);
//
//    struct MemoryAccess output = unbox_place(U, &x->output);
//    discharge_access(U, &output);
////    paw_assert(output.base.count == 1);
//    x->output = output.base;
//
//    x->mid = next_mid(U->fs);
...
////    MirPlaceList_reserve(mir, values, value.base.count);
////    values->count = value.base.count;
////    for (int i = 0; i < value.base.count; ++i) {
////        struct MirPlace const r = place_at(U, value.base, i);
...
//
//
//    struct MemoryAccess value = unbox_place(U, &x->value);
//    discharge_access(U, &value);
//
//    NEW_INSTR(U, return, x->loc, value.base);
//}
//
...
//        struct MirPlace const place = get_registers(U, *pr, ENUM_BASE);
//        MirRegisterList_push(mir, locals, place.r);
//    }
//
//    struct MirCaptureInfo const *pc;
//    MirCaptureList *captured = MirCaptureList_new(mir);
//    K_LIST_FOREACH (mir->captured, pc) {
//        struct MirPlace const group = get_registers(U, pc->r, ENUM_BASE);
...
//                if (plocal->value == backing->r.value) break;
//            }
