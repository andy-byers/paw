...
DeclId pawHir_add_decl(struct Compiler *C, struct HirDecl *decl)
{
    ...
    const DeclId did = {.value = dm->decls->count}; // TODO: set modno at this time!
...
struct HirType *copy_type(struct Compiler *C, struct HirType *node);
struct HirPat *copy_pat(struct Compiler *C, struct HirPat *node);
...
DEFINE_COPY_LIST(pat, Pat)
#undef DEFINE_COPY_LIST
...
//}
//
//static void CopyMatchArm(struct Compiler *C, struct HirMatchArm *e, struct HirMatchArm *r)
//{
//    r->guard = copy_pat(C, e->guard);
//    r->result = COPY_BLOCK(C, e->result);
//}
//
//static void CopyMatchStmt(struct Compiler *C, struct HirMatchStmt *e, struct HirMatchStmt *r)
//{
//    r->arms = copy_stmt_list(C, e->arms);
//}
//
...
//}
//
//// NOTE: ImplDecl and other Decl types are never (and cannot be) copied this way because of fields that contain IrType (.subst, .self, etc.)
...
//    r->name = d->name;
//    if (d->tag != NULL) r->tag = copy_type(C, d->tag);
...
//}
//
//static void CopySwitchDiscr(struct Compiler *C, struct HirSwitchDiscr *e, struct HirSwitchDiscr *r)
//{
//    r->expect = e->expect;
//    r->target = copy_expr(C, e->target);
//}
...
//}
//
//static void CopyJumpStmt(struct Compiler *C, struct HirJumpStmt *s, struct HirJumpStmt *r)
//{
...
//{
//    r->name = p->name;
//    r->pat = copy_pat(C, p->pat);
//}
//
//static void CopyStructPat(struct Compiler *C, struct HirStructPat *p, struct HirStructPat *r)
//{
//    r->path = dup_path(C, p->path);
//    r->fields = copy_pat_list(C, p->fields);
//}
//
//static void CopyVariantPat(struct Compiler *C, struct HirVariantPat *p, struct HirVariantPat *r)
//{
//    r->index = p->index;
//    r->path = dup_path(C, p->path);
//    r->fields = copy_pat_list(C, p->fields);
//}
//
//static void CopyTuplePat(struct Compiler *C, struct HirTuplePat *p, struct HirTuplePat *r)
//{
//    r->elems = copy_pat_list(C, p->elems);
//}
//
//static void CopyBindingPat(struct Compiler *C, struct HirBindingPat *p, struct HirBindingPat *r)
//{
//    PAW_UNUSED(C);
//    r->name = p->name;
//}
//
//static void CopyPathPat(struct Compiler *C, struct HirPathPat *p, struct HirPathPat *r)
//{
//    PAW_UNUSED(C);
//    r->path = dup_path(C, p->path);
//}
...
//}
//
//static void CopyPathType(struct Compiler *C, struct HirPathType *p, struct HirPathType *r)
//{
//    r->path = dup_path(C, p->path);
//}
//
//static void CopyFuncPtr(struct Compiler *C, struct HirFuncPtr *p, struct HirFuncPtr *r)
//{
//    r->params = copy_type_list(C, p->params);
//    r->result = copy_type(C, p->result);
//}
//
//static void CopyTupleType(struct Compiler *C, struct HirTupleType *p, struct HirTupleType *r)
//{
//    r->elems = copy_type_list(C, p->elems);
//}
//
//struct HirType *copy_type(struct Compiler *C, struct HirType *node)
//{
//    struct HirType *result = pawHir_new_type(
...
//}
//
//#define COPY_TYPE_PTR(C, from, to) SET_NODE_TYPE(C, to, GET_NODE_TYPE(C, from));
//
//struct HirExpr *copy_expr(struct Compiler *C, struct HirExpr *node)
//{
...
//            C, node->hdr.line, HIR_KINDOF(node));
//    result->hdr.flags = node->hdr.flags;
//    result->hdr.name = node->hdr.name;
//    switch (HIR_KINDOF(node)) {
//#define DEFINE_FOLD(a, b) case kHir##a: \
//            Copy##a(C, HirGet##a(node), HirGet##a(result)); \
//            break;
...
//struct HirPat *copy_pat(struct Compiler *C, struct HirPat *node)
//{
//    struct HirPat *result = pawHir_new_pat(
//            C, node->hdr.line, HIR_KINDOF(node));
//    switch (HIR_KINDOF(node)) {
//#define DEFINE_FOLD(a, b) case kHir##a: \
//            Copy##a(C, HirGet##a(node), HirGet##a(result)); \
//            break;
//        HIR_PAT_LIST(DEFINE_FOLD)
//#undef DEFINE_FOLD
//    }
//    COPY_TYPE_PTR(C, node, result);
//    return result;
...
//};
//
//static struct IrTypeList *expand_type_list(struct IrTypeFolder *F, struct IrTypeList *list)
//{
//    struct Expander *E = F->ud;
//    if (list == NULL) return NULL;
//    struct IrTypeList *copy = pawIr_type_list_new(E->C);
//    for (int i = 0; i < list->count; ++i) {
//        struct IrType *type = pawIr_fold_type(F, list->data[i]);
//        pawIr_type_list_push(E->C, copy, type);
//    }
//    return copy;
//}
//
//static struct IrType *expand_tuple(struct IrTypeFolder *F, struct IrTuple *t)
//{
//    struct Expander *E = F->ud;
//    struct IrType *r = pawIr_new_type(E->C, kIrTuple);
//    IrGetTuple(r)->elems = pawIr_fold_type_list(F, t->elems);
//    return r;
//}
//
//static struct IrType *expand_func_ptr(struct IrTypeFolder *F, struct IrFuncPtr *t)
//{
//    struct Expander *E = F->ud;
//    struct IrType *r = pawIr_new_type(E->C, kIrFuncPtr);
//    IrGetFuncPtr(r)->params = pawIr_fold_type_list(F, t->params);
//    IrGetFuncPtr(r)->result = pawIr_fold_type(F, t->result);
//    return r;
//}
//
//static struct IrType *expand_func_def(struct IrTypeFolder *F, struct IrSignature *t)
//{
//    struct Expander *E = F->ud;
//    struct HirDecl *base = pawHir_get_decl(E->C, t->did);
//    if (t->types == NULL) {
//        struct IrType *result = pawIr_new_type(E->C, kIrSignature);
...
//}
//
//static struct IrType *expand_adt(struct IrTypeFolder *F, struct IrAdt *t)
//{
//    struct Expander *E = F->ud;
//    if (t->did.value <= PAW_TSTR) return IR_CAST_TYPE(t);
//    struct IrTypeList *types = pawIr_fold_type_list(F, t->types);
//    struct HirDecl *base = pawHir_get_decl(E->C, t->did);
//    return pawP_instantiate(F->C, base, types);
//}
//
//static struct IrType *expand_generic(struct IrTypeFolder *F, struct IrGeneric *t)
//{
//    struct Expander *E = F->ud;
//    if (E->subst == NULL) return IR_CAST_TYPE(t);
//    const struct IrTypeList *before = E->subst->before;
//    const struct IrTypeList *after = E->subst->after;
//    for (int i = 0; i < before->count; ++i) {
//        struct IrType *search = before->data[i];
//        if (IrIsGeneric(search)) {
//            struct IrGeneric *g = IrGetGeneric(search);
//            if (t->did.value == g->did.value) return after->data[i];
//        }
//    }
...
//
//
//pawIr_print_type(E->C,GET_NODE_TYPE(E->C,inst));
//printf("0: %s: %s\n",inst->hdr.name->text,paw_string(E->C->P,-1));
//paw_pop(E->C->P,1);
//
...
//    if (base->self != NULL) r->self = pawIr_fold_type(F, base->self);
...
//        pawHir_fold_stmt(F, HIR_CAST_STMT(r->body));
//    }
//
//    E->subst = subst->outer;
//}
...
//}
//
//static void do_expand(struct IrTypeFolder *F, struct HirFuncDecl *base, struct HirDecl *inst)
//{
//    struct Expander *E = F->ud;
//    struct IrType *base_type = pawIr_get_type(E->C, base->hid);
//    struct IrType *inst_type = pawIr_get_type(E->C, inst->hdr.hid);
//
//    struct Subst subst = {
//        .outer = E->subst,
//        .before = IrGetSignature(base_type)->types,
//        .after = IrGetSignature(inst_type)->types,
//    };
//
//    enter_binder(E, inst);
...
//}
//
//static struct HirDecl *find_decl(struct Expander *E, struct IrType *type)
//{
//    return pawHir_get_decl(E->C, IR_TYPE_DID(type));
//}
//
...
////    struct Expander *E = F->ud;
////
////    const DeclId did = HIR_PATH_RESULT(path);
////
////
////    for (int i = 0; i < path->count; ++i) {
////        struct HirSegment *ps = pawHir_path_get(path, i);
////        if (ps->types != NULL) {
////  //          struct IrTypeList *types = pawP_lower_type_list(E->C, E->m, E->)
////            ps->types = pawIr_fold_type_list(F, ps->types);
////            struct HirDecl *base = pawHir_get_decl(E->C, ps->did);
////            struct IrType *inst = pawP_instantiate(E->C, base, ps->types);
////            ps->did = IR_TYPE_DID(inst);
////        }
////    }
//}
//
...
//    struct IrTypeFolder *F = V->ud;
//    struct Expander *E = F->ud;
//    pawIr_fold_expr(F, e->target);
//
//    struct IrType *result;
//    struct IrType *self_type = GET_NODE_TYPE(E->C, e->target);
//    if (IrIsTuple(self_type)) {
//        struct IrTuple *tuple = IrGetTuple(self_type);
//        result = K_LIST_GET(tuple->elems, e->index);
//    } else if (e->is_index) {
//        // variant field selector, generated when lowering matches
//        result = pawIr_fold_type(F, pawIr_get_type(E->C, e->hid));
//
//    } else {
//        struct HirDecl *field = pawP_find_field(E->C, self_type, e->name);
//        result = pawP_instantiate_field(E->C, self_type, field);
//    }
//    pawIr_set_type(E->C, e->hid, result);
//    return PAW_FALSE;
//}
//
...
//
//    pawIr_fold_expr(F, e->target);
//    pawIr_fold_expr_list(F, e->args);
//    return PAW_FALSE;
//}
//
//static struct IrTypeList *collect_decl_types(struct Hir *hir, struct HirDeclList *list)
//{
//    return pawHir_collect_decl_types(hir->C, list);
//}
//
...
//{
//    if (HirIsUseDecl(decl)) return; // TODO: 'HirUseDecl' should not exist probably
//    struct IrType *type = GET_NODE_TYPE(V->C, decl);
//    SET_NODE_TYPE(V->C, decl, pawIr_fold_type(V->ud, type));
//}
//
//static void expand_expr_type(struct HirVisitor *V, struct HirExpr *expr)
//{
//    struct IrType *type = GET_NODE_TYPE(V->C, expr);
//    SET_NODE_TYPE(V->C, expr, pawIr_fold_type(V->ud, type));
//}
//
...
//        .C = C,
//    };
//    pawIr_type_folder_init(&E->F, C, E);
//
//    E->F.FoldAdt = expand_adt;
//    E->F.FoldTuple = expand_tuple;
//    E->F.FoldSignature = expand_func_def;
...
struct IrTypeList *pawHir_collect_decl_types(struct Compiler *C, struct HirDeclList *list)
{
    ...
    for (int i = 0; i < list->count; ++i) {
        ...
        pawIr_type_list_push(C, types, GET_NODE_TYPE(C, decl));
    }
    return types;
}
...
struct IrTypeList *pawHir_collect_expr_types(struct Compiler *C, struct HirExprList *list)
{
    ...
    for (int i = 0; i < list->count; ++i) {
        ...
        pawIr_type_list_push(C, types, GET_NODE_TYPE(C, expr));
    }
    return types;
}
