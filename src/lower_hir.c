...
static struct LocalVar *add_local(struct FunctionState *fs, struct HirIdent ident, struct MirPlace r)
{
    ...
    MirRegisterList_push(fs->mir, fs->locals, r.r); // TODO: is .locals even necessary???
    ++fs->nlocals;
...
static MirBlock enter_function(struct LowerHir *L, struct FunctionState *fs, struct BlockState *bs, struct Mir *mir)
{
    *fs = (struct FunctionState){
    ...
    K_LIST_FOREACH (fptr->params, pparam) {
        ...
        int const size = IrIsTuple(*pparam) ? layout.size : 1; // TODO: unboxed enum
        mir->param_size += size;
    }
...
static paw_Bool is_pointer(struct FunctionState *fs, IrType const *type)
{
//    IrType const *type = mir_reg_data(fs->mir, r)->type;
...
static void auto_deref(struct FunctionState *fs, struct MirPlace *pplace, IrType const *target)
{
...
static void select_field(struct FunctionState *fs, struct MirPlace *pplace, IrType const *target, int index)
{
    auto_deref(fs, pplace, target);
    MirProjection *field = MirProjection_new_field(fs->mir, index);
    MirProjectionList_push(fs->mir, pplace->projection, field);
}

static void select_element(struct FunctionState *fs, struct MirPlace *pplace, IrType const *target, MirRegister index)
{
    auto_deref(fs, pplace, target);
    MirProjection *elem = MirProjection_new_index(fs->mir, index);
    MirProjectionList_push(fs->mir, pplace->projection, elem);
}
...
static void lower_place_aux(struct HirVisitor *V, struct HirExpr *expr, struct MirPlace *pplace)
{
    ...
    if (HirIsPathExpr(expr)) {
        *pplace = lower_path_expr(V, HirGetPathExpr(expr));
        ...
        select_field(fs, pplace, target, x->index);
        ...
        IrType *target = GET_NODE_TYPE(fs->C, x->target);
        select_element(fs, pplace, target, index.r);

      //  if (!x->is_slice) {
      //      select_element(fs, pplace, index.r);
      //  } else {
      //      lower_place_aux(V, x->target, pplace);
      //      struct MirPlace second = lower_place(V, x->second);
      //      struct MirPlace upper = new_place(fs, GET_NODE_TYPE(fs->C, x->second));
      //      move_to(fs, NODE_START(x->second), second, upper);
      //      select_range(fs, pplace, index.r, upper.r);
      //  }
    } else {
...
static void collect_binding(struct HirVisitor *V, struct HirBindingPat *p)
{
    ...
    // TODO
 //   struct MirPlace const r = place_for_node(fs, p->hid);
...
static void unpack_bindings(struct HirVisitor *V, struct HirPat *lhs, struct HirExpr *rhs)
{
    ...
    struct IrType *unit_type = pawIr_get_type(L->C, (HirId){0});
...
static struct MirPlace lower_tuple_lit(struct HirVisitor *V, struct HirLiteralExpr *e)
{
    ...
    IrType const *type = pawIr_get_type(fs->C, e->hid);
    ...
    K_LIST_ENUMERATE (e->tuple.elems, index, pexpr) {
        struct MirPlace place = pawMir_copy_place(fs->mir, output);
        select_field(fs, &place, type, index);
...
static struct MirPlace lower_composite_lit(struct HirVisitor *V, struct HirLiteralExpr *e)
{
    ...
    K_LIST_FOREACH (e->comp.items, pexpr) {
        ...
        struct MirPlace place = pawMir_copy_place(fs->mir, output);
        select_field(fs, &place, type, field->fid);
...
static struct MirPlace lower_container_lit(struct HirVisitor *V, struct HirLiteralExpr *e)
{
    ...
    K_LIST_ENUMERATE (e->cont.items, index, pexpr) {
        ...
        struct MirPlace place = pawMir_copy_place(fs->mir, output);
        select_element(fs, &place, type, key.r);
...
static struct MirPlace lower_unit_variant(struct HirVisitor *V, struct HirPathExpr *e, struct HirVariantDecl *d)
{
    ...
    struct MirPlace place = pawMir_copy_place(fs->mir, object);
    select_field(fs, &place, type, 0);
...
static struct MirPlace lower_path_expr(struct HirVisitor *V, struct HirPathExpr *e)
{
    ...
    if (resolve_nonglobal(fs, last.ident.name, &ng)) {
       ...
       //     return output;
       // } else {
       //     struct MirPlace place = ng.r;
       //     place.projection = MirProjectionList_new(fs->mir);
       //     return place;
        }
...
static void emit_get_field(struct FunctionState *fs, struct SourceLoc loc, struct MirPlace object, int index, struct MirPlace output)
{
...
static struct MirPlace lower_concat_expr(struct HirVisitor *V, struct HirBinOpExpr *e)
{
    struct LowerHir *L = V->ud;
    struct FunctionState *fs = L->fs;
...
static struct MirPlace lower_variant_constructor(struct HirVisitor *V, struct HirCallExpr *e, struct HirVariantDecl *d)
{
    ...
    struct IrLayout variant_layout = IrLayoutList_get(layout.fields, d->index);
    ...
    K_LIST_ENUMERATE (e->args, index, pexpr) {
        ...
        struct MirPlace place = pawMir_copy_place(fs->mir, object);
        select_field(fs, &place, type, 1 + index);
        move_to(fs, e->span.start, discr, place);
    }
...
//
//    struct MirPlace const object = place_for_node(fs, e->hid);
//    struct IrLayout layout = pawMir_get_layout(fs->mir, object);
//    NEW_INSTR(fs, aggregate, e->span.start, layout.size, object);
//
//    struct MirPlace const discr = add_constant(fs, d->span.start, I2V(d->index), BUILTIN_INT);
//
...
static void allocate_match_vars(struct FunctionState *fs, struct MirPlace object, struct MatchCase mc, paw_Bool is_enum)
{
    if (mc.vars->count == 0)
        return;

    IrType const *type = mir_reg_data(fs->mir, object.r)->type;
    struct IrLayout layout = pawMir_get_layout(fs->mir, object.r);
    ...
    K_LIST_ENUMERATE (mc.vars, index, pv) {
        struct MirPlace source = pawMir_copy_place(fs->mir, object);
        struct MirPlace const r = new_place(fs, pv->type);
        map_var_to_reg(fs, *pv, r);

        select_field(fs, &source, type, is_enum + index);
        move_to(fs, pv->span.start, source, r);
//    emit_get_field(fs, pv->span.start, object, elem.offset, r);

//        emit_get_field(fs, pv->span.start, object, is_enum + index, r);
