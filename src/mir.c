...
struct MirSwitchArm *pawMir_new_switch_arm(struct Compiler *C, int value, MirBlockId bid)
{
    struct MirSwitchArm *r = pawK_pool_alloc(ENV(C), C->pool, sizeof(struct MirSwitchArm));
    *r = (struct MirSwitchArm){
        .value = value,
        .bid = bid,
    };
    return r;
