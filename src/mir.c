...
static void filter_usedef_map(struct Compiler *C, Map *mapping, Map *usedef)
{
    int index;
    MirBlock *pb;
    ...
    while (pawH_iter(usedef, &iter)) {
        ...
        if (blocks->count == 0) pawH_erase_at(usedef, iter);
    }
}
...
void pawMir_remove_unreachable_blocks(struct Compiler *C, struct Mir *mir, Map *uses, Map *defs)
{
    ...
    for (int i = 0; i < order->count; ++i) {
        const MirBlock b = K_LIST_GET(order, i);
    ...
    filter_usedef_map(C, map, uses);
    filter_usedef_map(C, map, defs);
