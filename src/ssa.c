...
struct SsaConverter {
    ...
    Map *uses; // vid => [MirBlockList]
    Map *defs; // vid => [MirBlockList]
    int location;
    int N;
};
...
static MirRegister new_register(struct SsaConverter *S, struct IrType *type, MirRegister old)
{
    ...
    K_LIST_PUSH(S->C, S->registers, ((struct MirRegisterData){type}));
...
static void rename_instruction(struct SsaConverter *S, struct MirInstruction *instr)
{
    ...
    if (pawMir_check_store(S->C, instr, &store)) {
        K_LIST_FOREACH(store.outputs, ppr) rename_output(S, *ppr);
    }
}
