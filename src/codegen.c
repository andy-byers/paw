...
#define BB(G, bid) (K_LIST_GET((G)->mir->blocks, (bid).value))
...
// TODO: already know the number of basic blocks in each function. allocate space for a
//       JumpTarget for each basic block when the function is entered, fill in as code
//       is generated
DEFINE_LIST(struct Compiler, jumptab_, JumpTable, struct JumpTarget)

static struct JumpTarget *add_jump_target(struct Generator *G, MirBlockId bid)
{
    struct JumpTarget *jt = pawK_pool_alloc(ENV(G), G->pool, sizeof(struct JumpTarget));
    *jt = (struct JumpTarget){
        .pc = G->fs->pc,
        .bid = bid,
    };
    K_LIST_PUSH(G->C, G->jumps, jt);
    return jt;
}
...
{
    struct VisitBlock *vb = pawK_pool_alloc(ENV(G), G->pool, sizeof(struct VisitBlock));
    *vb = (struct VisitBlock){
...
{
    struct JumpSource *js = pawK_pool_alloc(ENV(G), G->pool, sizeof(struct JumpSource));
    *js = (struct JumpSource){
        .from_pc = from_pc,
        .to = to,
    };
    K_LIST_PUSH(G->C, G->patch, js);
}
...
static void patch_jumps_to_here(struct Generator *G, MirBlockId bid)
{
    ...
    for (int i = 0; i < pl->count;) {
        struct JumpSource *js = pl->data[i];
...
static void mangle_types(struct Generator *G, Buffer *buf, struct IrTypeList *types)
{
...
static String *mangle_attr(struct Generator *G, const String *modname, const String *base, struct IrTypeList *base_types, const String *attr, struct IrTypeList *attr_types)
{
    Buffer buf;
    paw_Env *P = ENV(G);
    mangle_start(P, &buf, G);
...
{
    struct IrSignature *fdef = IrGetSignature(type);
    struct HirFuncDecl *fd = HirGetFuncDecl(GET_DECL(G, fdef->did));
    struct IrTypeList *fd_types = fd->body ? fdef->types : NULL;
    if (fd->self == NULL) return mangle_name(G, modname, fd->name, fd_types);
    struct HirAdtDecl *ad = HirGetAdtDecl(GET_DECL(G, IR_TYPE_DID(fd->self)));
    struct IrTypeList *ad_types = fd->body ? ir_adt_types(fd->self) : NULL;
...
static int add_nested_proto(struct Generator *G, Proto *parent, Proto *p)
{
    struct FuncState *fs = G->fs;
    if (fs->nproto == ITEM_MAX) {
        ERROR(G, PAW_ESYNTAX, "too many functions");
    } else if (fs->nproto == parent->nproto) {
        pawM_grow(ENV(G), parent->p, fs->nproto, parent->nproto);
        for (int i = fs->nproto; i < parent->nproto; ++i) {
            parent->p[i] = NULL;
        }
    }
...
static struct VarInfo new_local(struct FuncState *fs, String *name, struct IrType *type)
{
...
static struct VarInfo new_local_lit(struct Generator *G, const char *name, paw_Type code)
{
    ...
    const DeclId did = {.value = code};
    struct HirDecl *decl = GET_DECL(G, did);
    return new_local(G->fs, str, GET_NODE_TYPE(G->C, decl));
}
...
static void code_proto(struct Generator *G, struct Mir *mir, Proto *proto, int index)
{
    ...
    G->regtab = pawP_allocate_registers(G->C, mir, order); // push 'G->regtab'
...
//    const String *modname = prefix_for_modno(G, IR_TYPE_DID(type).modno);
//    const String *name = func_name(G, modname, type);
...
static void code_free_local(struct MirVisitor *V, struct MirFreeLocal *x)
{
    struct Generator *G = V->ud;
    struct FuncState *fs = G->fs;
    if (x->is_captured) pawK_code_A(fs, OP_CLOSE, REG(fs, x->reg));
}
...
static paw_Bool code_switch(struct MirVisitor *V, struct MirSwitch *x)
{
    ...
    for (int i = 0; i < x->arms->count; ++i) {
        ...
        const int next_jump = code_switch_int(fs, x->discr, arm->value);
...
void pawMir_dump_signatures(struct Compiler *C, Map *bodies)
{
    printf("%zu MIR bodies:\n", pawH_length(bodies));
    paw_Int iter = PAW_ITER_INIT;
    while (pawH_iter(bodies, &iter)) {
        struct Mir *mir = pawH_value(bodies, iter)->p;
        const char *type = pawIr_print_type(C, mir->type);
        printf("  %s: %s\n", mir->name->text, type);
        paw_pop(ENV(C), 1);
    }
}
