...
{
    paw_assert(n == 2); // TODO
    StackPtr ra = P->top.p - 2; // TODO
    const StackPtr rb = P->top.p - 2; // TODO
    const StackPtr rc = P->top.p - 1; // TODO

    const String *x = V_STRING(*rb);
    const String *y = V_STRING(*rc);
...
void pawR_list_concat(paw_Env *P, CallFrame *cf, int n)
{
    ...
    const List *x = V_LIST(*rb);
    const List *y = V_LIST(*rc);
    List *z = VM_LIST_INIT(ra);
