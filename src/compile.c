...
static struct Type *new_type(struct DefGenerator *dg, struct IrType *src, DefId did)
{
    ...
    switch (IR_KINDOF(src)) {
        ...
        case kIrFuncPtr: {
            ...
            init_type_list(dg, fptr->params, dst->subtypes, did);
            dst->sig.result = MAKE_TYPE(dg, fptr->result, did);
            break;
        }
        default: { // kIrTuple
            struct IrTuple *tuple = IrGetTuple(src);
            dst = pawY_new_tuple(P, tuple->elems->count);
            init_type_list(dg, tuple->elems, dst->subtypes, did);
        }
    }
    return dst;
}
...
static void allocate_items(struct DefGenerator *dg, struct MirBodyList *bodies)
{
    for (int i = 0; i < bodies->count; ++i) {
        ...
        K_LIST_APPEND(dg->C, dg->items, item);
...
{
    paw_assert(!pawP_is_assoc_fn(C, method) || get_self(C, method) == self);
