...
#define DEFINE_MAP(Context_, Name_, Alloc_, Hash_, Equals_, Key_, Value_) \
    ...
    static inline int Name_##_length(const struct Name_ *map) \
    { \
        return map->count; \
    } \
    ...
    { \
        ...
        while (alloc <= (size_t)map->alloc) alloc *= 2; \
...
#define DEFINE_MAP_ITERATOR(Name_, Key_, Value_) \
    ...
    static inline paw_Bool Name_##Iterator_is_valid(const Name_##Iterator *iter) \
    { \
