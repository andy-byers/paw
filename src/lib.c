...
{
    return paw_get_count(P) - 1 /* callee */;
}
...
{
    pawL_check_argc(P, 2);
    const String *str = pawV_get_string(cf_base(1));
    const char *prefix = pawL_check_string(P, 2);
    const size_t length = paw_length(P, 2);
...
{
    pawL_check_argc(P, 3);
    const paw_Int i = pawL_check_int(P, 2);
    Array *a = pawV_get_array(cf_base(1));
    pawA_insert(P, a, i, cf_base(3));
    return 0;
}

static int array_push(paw_Env *P)
{
    const int argc = pawL_check_varargc(P, 2, UINT8_MAX);
    Array *a = pawV_get_array(cf_base(1));
    for (int i = 1; i < argc; ++i) {
        pawA_push(P, a, cf_base(1 + i));
    }
    return 0;
}
...
static int array_pop(paw_Env *P)
{
    ...
    const paw_Int i = argc == 2 ? pawV_get_int(cf_base(2)) : -1;
...
{
    pawL_check_argc(P, 2);
    Map *m = pawV_get_map(cf_base(1));
    pawH_remove(P, m, cf_base(2));
    return 0;
}
