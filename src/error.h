...
#define ERR_ERROR_LIST(X) \
    ...
    X(TooManyLines, ERR_CATEGORY_SYNTAX, \
        struct Str const *modname; \
        struct Str const *filename; \
        int max_lines; \
    ) \
    ...
    X(ExpectedSymbol, ERR_CATEGORY_SYNTAX, \
        struct Str const *modname; \
        struct SourceSpan span; \
        struct Str const *expected; \
        struct Str const *have; \
    ) \
    ...
    X(AmbiguousPath, ERR_CATEGORY_SYNTAX, \
        struct Str const *modname; \
        struct SourceSpan span; \
        struct Str const *path; \
    ) \
    X(ExternFunctionBody, ERR_CATEGORY_SYNTAX, \
        struct Str const *modname; \
        struct SourceSpan span; \
        struct Str const *fn_name; \
    ) \
    ...
    X(AssociatedItemVisibility, ERR_CATEGORY_SYNTAX, \
        struct Str const *modname; \
        struct SourceSpan span; \
        struct Str const *parent_name; \
        struct Str const *field_name; \
    ) \
    ...
    X(TraitImplMissingAssocItem, ERR_CATEGORY_NAME, \
        struct Str const *modname; \
        struct Str const *missing_modname; \
    ...
    X(FalseObligation, ERR_CATEGORY_TYPE, \
        struct Str const *modname; \
        struct SourceSpan span; \
        struct Str const *obligation; \
        struct Str const *cause; \
    ) \
    ...
    X(IncompatibleTypes, ERR_CATEGORY_TYPE, \
        struct Str const *modname; \
        struct SourceSpan span; \
        struct Str const *lhs; \
        struct Str const *rhs; \
    ) \
...
typedef struct ErrorHandler {
    ...
    struct Str const *modname;
    struct Str const *message;
    struct Str const *hint;
    struct SourceSpan span;
} ErrorHandler;

void pawErr_start(paw_Env *P);
void pawErr_set_module_name(paw_Env *P, struct Str const *name);
...
PAW_NORETURN void pawErr_generic_error(paw_Env *P, struct Str const *modname, struct SourceSpan span, char const *fmt, ...);

// Convenience functions for throwing common errors
PAW_NORETURN void pawErr_exceeded_limit(paw_Env *P, struct Str const *modname, struct SourceSpan span, char const *what, paw_Int limit);
